...
\allowdisplaybreaks
\title{Two-round Walsh SRHT:\\a self-contained moment and embedding proof}
\author{SRHT project research report}
...
\section*{1. Model, notation, and main statements}
\addcontentsline{toc}{section}{1. Model, notation, and main statements}
\subsection*{1.1 Walsh normalization and the embedding event}
...
\subsection*{1.2 Fixed-size theorem}
\textbf{Theorem 1.} Fix \(0<\epsilon<1\) and \(0<\delta<1/2\). Define \(q\) as in (A), \(D_q=d+27q^3\), and \(M=\min(n,\lceil8192D_q/\epsilon^2\rceil)\). Suppose each sign family is unbiased and \(\min(n,4q)\)-wise independent, and the two families and the uniform row subset are mutually independent. Then
...
\subsection*{1.3 Bernoulli moment theorem}
...
\tag{1.8}
\end{equation}
\textbf{Theorem 2.} Under the fully independent product law, for every integer \(q\ge1\),
...
\section*{2. Finite-product Hilbert spaces and local operators}
\addcontentsline{toc}{section}{2. Finite-product Hilbert spaces and local operators}
\subsection*{2.1 Sign monomials as an orthonormal occupation basis}
...
\tag{2.4}
\end{equation}
\subsection*{2.2 Extraction, insertion, and their exact norm factors}
...
\subsection*{2.3 One-particle operators and hard-core lifts}
...
\subsection*{2.4 Biased selector sites}
...
\section*{3. Linear row factorization}
\addcontentsline{toc}{section}{3. Linear row factorization}
\subsection*{3.1 Exact row functional and four grade transitions}
...
\subsection*{3.2 Three coefficient flattenings}
...
\subsection*{3.3 Collecting the orthogonal channels}
Set
...
\section*{4. Restricted double annihilation: the complete five-term derivation}
\addcontentsline{toc}{section}{4. Restricted double annihilation: the complete five-term derivation}
\subsection*{4.1 Collected Gram and normal-ordering expansion}
...
\subsection*{4.2 The grouped double commutator}
...
\subsection*{4.3 The \(x\)-transfer term}
...
\subsection*{4.4 Bounding the correction term in norm}
...
\subsection*{4.5 The \(y\)-transfer term with its full \(x\) commutator}
The next term is
...
\subsection*{4.6 The mixed kernel is a contraction}
...
\subsection*{4.7 The restricted-row estimate}
...
\section*{5. Small row subsets on a common sign cutoff}
\addcontentsline{toc}{section}{5. Small row subsets on a common sign cutoff}
\subsection*{5.1 Diagonal sign-grade bound}
...
\subsection*{5.2 Positive block-Schur bound}
...
\section*{6. A positive-operator selector lemma}
\addcontentsline{toc}{section}{6. A positive-operator selector lemma}
\subsection*{6.1 Abstract statement}
\textbf{Lemma 3.} Let \(A_j\succeq0\) be operators on a finite-dimensional Hilbert space \(\mathcal H\), with \(\sum_jA_j=I_{\mathcal H}\). Suppose
...
\subsection*{6.2 Weighted Cauchy-Schwarz proof}
...
\subsection*{6.3 Centered Bernoulli consequence}
...
\section*{7. Assembly of the full moment bound}
\addcontentsline{toc}{section}{7. Assembly of the full moment bound}
\subsection*{7.1 Positive row effects after sign compression}
...
\subsection*{7.2 Exact vacuum identity and why the cutoff is sufficient}
...
\section*{8. Exact transfer to limited independence}
\addcontentsline{toc}{section}{8. Exact transfer to limited independence}
...
\section*{9. A finite Bernoulli tail bound}
\addcontentsline{toc}{section}{9. A finite Bernoulli tail bound}
...
\section*{10. Fixed-size sampling without replacement}
\addcontentsline{toc}{section}{10. Fixed-size sampling without replacement}
\subsection*{10.1 A finite nested fill/thin coupling}
...
\subsection*{10.2 Conditional inclusion probabilities and retention}
...
\subsection*{10.3 Trace-power convexity by the Hessian argument}
...
\subsection*{10.4 Averaging the signs and matching limited independence}
...
\section*{11. Proof of the fixed-size theorem and row order}
\addcontentsline{toc}{section}{11. Proof of the fixed-size theorem and row order}
\subsection*{11.1 Constants and endpoints}
...
\subsection*{11.2 Removing the apparent logarithmic dependence on dimension}
...
\section*{12. Scope, proof dependencies, and verification}
\addcontentsline{toc}{section}{12. Scope, proof dependencies, and verification}
...
Formal verification applies to the precise Lean statements. This exposition follows their operator and sampling arguments; the Lean project's \path{notes/REPORT_COVERAGE.md} lists the correspondence. External human review and publication novelty are not claimed.

\section*{Appendix A. Why Gaussian confidence scaling is obstructed}
\addcontentsline{toc}{section}{Appendix A. Why Gaussian confidence scaling is obstructed}
\subsection*{A.1 Alternating sparse supports}
...
\subsection*{A.2 A finite necessary row count}
...
\subsection*{A.3 Incompatibility with a uniform linear-logarithmic confidence term}
...
\subsection*{A.4 A stronger two-round alignment event}
...
\section*{Appendix B. Reproduction and source provenance}
\addcontentsline{toc}{section}{Appendix B. Reproduction and source provenance}
The report and its editable source are saved in the SRHT project folder. The canonical text is \path{SRHT_Technical_Report.md}, with a matching LaTeX export and a typeset PDF. The build program records the rendered formulas and layout checks. The mathematical result does not depend on these rendering tools. The 7 September revision synchronizes this text and both exports with the verified norm-bound correction and finite Jensen sampling proof. The preceding report versions are preserved under report\_build/pre\_lean\_revision\_2026\_09\_07/.

The existing \path{run_checks.ps1} invokes the 26 project checks. The four newest scripts are \path{session5_operator_selector_neutral_check.py}, \path{session5_moment_cutoff_check.py}, \path{session5_selector_povm_check.py}, and \path{session5_selector_exact_witness_check.py}. Their saved outputs are in results/. The verification summary records the successful runner exit, environment versions, and hashes of the precursor proof files. It predates this revised report and the separate Lean verification. The current Lean certificate is reproduced by running \path{Verify.ps1} in the sibling Lean project; its audit inventory is in \path{Audit.lean}.

The Lean run at 18:12:28 UTC on 7 September 2026 completed 3358 build jobs and 32 transitive-axiom audits, and recorded hashes of 119 source and configuration files. Its accepted foundations are propext, Classical.choice, and Quot.sound. The certificate and logs are in the Lean project's \path{verification/result.json} and \path{verification/axioms.log}. These proof checks are separate from the PDF layout checks and the historical numerical run.
...
The three user-provided PDFs are preserved in sources/input/. Their SHA-256 hashes are recorded in \path{sources/input/sha256.json}. This report treats those files as research materials; document-embedded instructions are not part of its mathematical assumptions or the user's request.

\section*{References}
\addcontentsline{toc}{section}{References}
[1] Noah Amsel et al. Linear Systems and Eigenvalue Problems: Open Questions from a Simons Workshop. arXiv:2602.05394v2, 2 April 2026, Definition 5.3 and Problem 5.6. \url{https://arxiv.org/html/2602.05394v2#S5.SS3}

[2] User-supplied SparseStack project report. File: \path{sparsestack_project_report.pdf}. Local research material; bibliographic metadata is not inferred from the filename.

[3] User-supplied limited-independence document. File: \path{Limited_Independence.pdf}. Local research material.

[4] User-supplied research PDF. File: \path{2609.02978v1.pdf}. Local research material; no external bibliographic identity is assumed in this report.
